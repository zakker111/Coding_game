...
export function renderFrame(params) {
  ...
  for (const b of snapshot.bullets ?? []) {
    ...
    // slight intra-tick offset in direction for readability
...
function drawGrid(ctx, ox, oy, arenaPx) {
  // zone lines every 32, sector lines every 64
  const zone = arenaPx / 6; // 192* S / 6 = 32*S
  const sector = zone * 2;
...
function drawBot(ctx, cx, cy, bot, S) {
  ...
  ctx.fillRect(cx - 18*S/2, cy - r - 16, 36, 14);
