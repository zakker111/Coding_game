...
export function createReplay(params) {
  ...
  for (let t = 1; t <= params.tickCap; t++) {
    ...
    // - Shoot if within range and ammo/cooldown allow.
    for (const bot of st.bots) {
      ...
      // Shooting intent: if enemy in close-ish range
      const target2 = selectClosestEnemy(st, bot);
      if (target2) {
        const dist = distanceMap(bot.loc).get(`${target2.loc.sector}:${target2.loc.zone}`) ?? 999;
        // cooldown stored on bot.pc for now would be wrong; keep per-bot simple fireCooldown using pc for skeleton? No.
        // We'll store an ad-hoc cooldown as part of bot state via a hidden field.
      }

      // For this skeleton: emulate a simple weapon cooldown by using `pc` as a counter (kept deterministic).
      // pc counts down after firing, otherwise stays.
      // This is not the final DSL pc; it’s a placeholder until the real VM is implemented.
      ...
      if (tgt) {
        ...
        if (canFire) {
          ...
          bot.pc = 1 + rules.bulletCooldownOnUseTicks; // placeholder cooldown store
    ...
    // Phase 6: Pickups
    // If bot stands on powerup anchor, pick it up.
    ...
    // Decrement movement cooldowns and (placeholder) fire cooldown stored in pc already handled above.
    for (const bot of st.bots) {
