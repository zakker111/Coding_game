...
// Stable v1 SHIELD numbers (match `packages/replay/src/generateSampleReplay.js`).
...
function buildObservation(bot, bots, bullets, powerupState) {
  ...
  return {
    ...
    // Slots
    hasModule: (slot) => slot === 1 || (slot === 2 && bot.shieldCapable),
...
function stepToggleDrains(bots, tickEvents) {
  for (const bot of bots) {
    ...
    if (!bot.sawActive && !bot.shieldActive) continue
